...
Ver también [`README.md`](README.md), [`ARQUITECTURA.md`](ARQUITECTURA.md) —cómo funciona por dentro—
y [`CONTRIBUIR.md`](CONTRIBUIR.md) —cómo se trabaja en él—.
...
| Concurrencia | **Un solo hilo motor** con scheduler cooperativo de generadores. 3 hilos en el camino de entrada: Tk, hook, motor (los otros dos del proceso, `sonidos` y el efímero `guardado`, solo existen para no bloquear a nadie) | Un único emisor de `SendInput` da orden determinista, un solo `finally` que lo suelta todo y ninguna carrera entre generadores. El vigilante de foco y la sonda son tareas del mismo scheduler, no hilos aparte. |
...
| Lo que no se entiende | Se conserva su JSON original en `Macro.bruto` y se devuelve intacto al guardar, en vez de descartarlo | Una macro ilegible sigue siendo trabajo del usuario. Al serializar solo las soportadas, editar cualquier otra cosa la borraba en silencio: bastaba un dedazo editando el fichero a mano, o abrir la configuración con una versión anterior del programa, para perderla. Y es justo lo que hace innecesario subir `VERSION_CONFIG` cada vez que se añade un tipo de acción. |
